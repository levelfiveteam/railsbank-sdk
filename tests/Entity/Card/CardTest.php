...
class CardTest extends TestCase
{
    public function testBeneficiary()
    {
        ...
        self::assertEquals('card-status-active', $entity->getCardStatus());
        self::assertEquals('UK',$entity->getCardProgramme());

        self::assertTrue($entity->isActive());
    }

    public function testCardIsActive()
    {
        $response = [
            'ledger_id' => '1234',
            'card_token' => 'GB1234111000',
        ...
        self::assertEquals('product-a', $entity->getPartnerProduct());
        self::assertEquals('thebest', $entity->getCardDesign());
        self::assertEquals('card-status-awaiting-activation', $entity->getCardStatus());
        self::assertEquals('UK',$entity->getCardProgramme());

        self::assertTrue($entity->isAwaitingActivation());
    }
}
